;(function () {
  ...
  class MiniVue {
    constructor(options) {
      ...
      if (this.$el) {
        // 数据劫持
        this.$data = new Observer().walk(this.$data)
  ...
  class Observer {
    ...
    walk(data) {
      ...
      if (data && typeof data === 'object') {
        // 使用proxy代理拦截监听data中的数据变化
        // 初始化 发布订阅
        let emitter = new Emitter()

        let handler = {
          get(target, key) {
            // 递归判断当前data中的数据是否是对象，如果是对象，就继续使用proxy代理拦截监听data中的数据变化
            ...
            // 判断当前是否有发布订阅状态
            if (Emitter.target) {
              // 如果有发布订阅状态，就调用发布订阅中的回调函数
              emitter.addSub(Emitter.target)
            }
            // 返回data中的数据
            return target[key]
          },
